          ...
          const formsData = await Promise.all(
            speciesDetails.varieties.map(async (variety) => {
              ...
              const pokemonData = {
                ...
                isLegendary: speciesDetails.is_legendary, // Store isLegendary
                isMythical: speciesDetails.is_mythical, // Store isMythical
                generation: speciesDetails.generation.name // Store generation
              };
...
const SHOWDOWN_SPRITES_URL = 'https://play.pokemonshowdown.com/sprites/dex/';
const SHOWDOWN_SHINY_SPRITES_URL = 'https://play.pokemonshowdown.com/sprites/dex-shiny/';
          ...
          const formsData = await Promise.all(
            speciesDetails.varieties.map(async (variety) => {
              ...
              const pokemonData = {
                ...
                form: variety.is_default ? null : variety.pokemon.name,
                types: formDetails.types.map(typeInfo => typeInfo.type.name),
                sprite: `${SHOWDOWN_SPRITES_URL}${formDetails.name.toLowerCase()}.png`,
                shinySprite: `${SHOWDOWN_SHINY_SPRITES_URL}${formDetails.name.toLowerCase()}.png`,
                height: formDetails.height,
                weight: formDetails.weight,
                pokedexEntries: pokedexEntries,
                abilities: abilities, // Store ability objects with ID + name
